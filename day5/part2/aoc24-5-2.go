...
func sumNumberOfMiddlePageOfIncorrectUpdatesAfterFixingThem(input string) int {
	// Parse input
...
func fixUpdate(rules map[int][]int, update []int) {
	...
	for i := 0; i < updateLength-1; i++ {
		for j := i + 1; j < updateLength; j++ {
			if !isPageCorrect(rules, update[i], update[j]) {
				...
				j = i // what we really want is j = i+1 but j++ will happen
			}
		}
	}
}
...
func main() {
	...
	fmt.Println(sumNumberOfMiddlePageOfIncorrectUpdatesAfterFixingThem(input))
}
